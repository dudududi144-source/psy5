import { clamp, mulberry32, loopLen, M_ENERGY, M_DRIVE, M_SPACE, M_MOVE, M_FILTER, M_TIGHT, M_HAUNT, M_FAZE, tapTempo, fillEvents, FILL_NAMES } from './model.js';
import { findTransTrack } from './transition.js';
...
import { DEFAULT_KIT, kitPatch } from '../foundation/dsp/kit-reason.mjs';
...
autoOn:false,autoQuant:true,autoArm:new Set(),selLane:-1,fillType:0};
...
const PERF={toggleLayer(which){const p=I.p;pushHist();if(which==='drums')for(let t=0;t<4;t++)p.tracks[t].mix.mute=!p.tracks[t].mix.mute;else if(which==='bass')p.tracks[4].mix.mute=!p.tracks[4].mix.mute;else if(which==='music')for(let t=5;t<p.tracks.length;t++)p.tracks[t].mix.mute=!p.tracks[t].mix.mute;else if(which==='fx')for(let t=4;t<p.tracks.length;t++)p.tracks[t].mix.sendA=p.tracks[t].mix.sendA>0?0:.35;after()},macro(idx,val){I.p.macroVals[idx]=clamp(val,0,1);resolveMacros();I.dirty=true;autoRecMove(-1,'macro.'+idx,I.p.macroVals[idx])},launch(i,instant){const p=I.p,sc=p.scenes[i];if(!sc||sc.pattern==null)return{ok:false};if(instant||!I.sched.on){pushHist();p.activeScene=i;p.currentPattern=sc.pattern;I.pending=null;/* v0.8.0 mix snapshot: applied on the instant path too (after() glides at currentTime below) */applySceneMix(p,i);after();/* per-scene auto-FILL: instant launch fires the FILL op right away (quantized launches fire it at the bar boundary — see scheduler) */if(sc.fill&&I.eng)PERF.fill()}else{I.pending=i;if(I.fsm==='PLAYING'||I.fsm==='RECORDING')I.fsm='TRANSITIONING';I.renderDirty=true}return{ok:true}},assign(i){pushHist();I.p.scenes[i].pattern=I.p.currentPattern;after()},fill(){const p=I.p;if(!I.eng||!I.ctx)return;/* v0.18.0: the fill LAYOUT is pure (fillEvents in model.js) — three cycling variants; missing tracks are skipped honestly */const sd=60/p.bpm/4,t0=I.ctx.currentTime;for(const e of fillEvents(I.fillType)){const tr=p.tracks[e.track];if(!tr)continue;I.eng.trigger(tr,t0+e.off*sd,{vel:e.vel,note:48,lock:e.lock||{}},sd)}},fillCycle(){I.fillType=(I.fillType+1)%FILL_NAMES.length;return I.fillType},/* v0.18.0 DJ TOOLS — the v0.16 transition voices (riser/revcym/impact) fired ON DEMAND, right now, for jams and manual builds. Carrier = the lowest drum track whose TYPE matches (findTransTrack — the same lookup the scene transitions use); a set without that voice type refuses HONESTLY (the caller toasts the Sound-tab fix) — never retypes or creates tracks. */dj(kind){const p=I.p;if(!I.eng||!I.ctx||!p)return{ok:false,reason:'offline'};const ti=findTransTrack(p,kind);if(ti<0)return{ok:false,reason:'no-'+kind};I.eng.trigger(p.tracks[ti],I.ctx.currentTime,{vel:kind==='impact'?.9:.82,note:48,lock:{}},0);return{ok:true}},tap(){const p=I.p;if(!p)return 0;const r=tapTempo(I.taps||[],Date.now());I.taps=(I.taps||[]).filter(x=>Date.now()-x<2500);I.taps.push(Date.now());if(r.bpm!=null){pushHist();p.bpm=r.bpm;after();toast('TAP TEMPO → '+r.bpm+' BPM')}return I.taps.length},variation(){const p=I.p;pushHist();const rng=mulberry32((Date.now()%100000)|0);const pat=p.patterns[p.currentPattern];[2,3].forEach(t=>{const d=pat.data[t];if(!d)return;for(let i=0;i<d.len;i++){const s=d.steps[i];if(s.on&&rng()<.15)s.on=0;else if(!s.on&&rng()<.12){s.on=1;s.vel=.45+rng()*.35}}});const d7=pat.data[7];if(d7)for(let i=0;i<d7.len;i++){const s=d7.steps[i];if(s.on)s.prob=rng()<.1?.6:1}after()},
/* v0.21.0 THROW TOOLS — the two classic DJ performance moves, zero engine
   change, quantized release:
   echo  — slam the master delay feedback to its ceiling (0.8, the
           delayFbClamp max) so whatever is playing smears into repeats;
   muffle— pull the master EQ high/mid shelves down (−12/−9 dB, the
           registry clamps) so the mix drops behind a veil.
   ARM: press once (or 'g'/'h'). RELEASE: press the SAME tool again — or
   let the armed barHook auto-release after 2 bar boundaries (the
   scheduler fires barHooks only while playing; a stopped transport holds
   the throw until the next play, which is exactly how a held DJ move
   behaves). Momentary by design: no pushHist, no automation write — the
   release restores the exact pre-throw values, so the project state is
   net-identical unless the user SAVES mid-throw. */
throwFx(kind){
const p=I.p;if(!p)return{ok:false,reason:'offline'};
const stateKey=kind==='echo'?'_throwE':kind==='muffle'?'_throwM':null;
if(!stateKey)return{ok:false,reason:'bad-kind'};
if(I[stateKey]){ /* second press — release NOW */
const sv=I[stateKey];I[stateKey]=null;const hi=I.barHooks.indexOf(sv.hook);if(hi>=0)I.barHooks.splice(hi,1);
if(kind==='echo')p.fx.delayFb=sv.orig;else{p.master.eqHigh=sv.orig.eqHigh;p.master.eqMid=sv.orig.eqMid}
after();toast(kind==='echo'?'ECHO THROW released':'MUFFLE released');
return{ok:true,released:true}}
let saved,apply;
if(kind==='echo'){saved=p.fx.delayFb;apply=()=>{p.fx.delayFb=.8}}
else{saved={eqHigh:p.master.eqHigh,eqMid:p.master.eqMid};apply=()=>{p.master.eqHigh=-12;p.master.eqMid=-9}}
let bars=2;
const hook=()=>{bars--;if(bars<=0){PERF.throwRelease(kind)}};
I[stateKey]={orig:saved,hook};apply();after();I.barHooks.push(hook);
toast(kind==='echo'?'ECHO THROW — 2 bars (again = release)':'MUFFLE — 2 bars (again = release)');
return{ok:true,released:false}},
throwRelease(kind){const stateKey=kind==='echo'?'_throwE':'_throwM';const sv=I[stateKey];if(!sv)return;I[stateKey]=null;const hi=I.barHooks.indexOf(sv.hook);if(hi>=0)I.barHooks.splice(hi,1);const p=I.p;if(!p)return;if(kind==='echo')p.fx.delayFb=sv.orig;else{p.master.eqHigh=sv.orig.eqHigh;p.master.eqMid=sv.orig.eqMid}after()}};
