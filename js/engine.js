...
/* ── v0.27.0 TRANSPARENT MASTERING (ported from psyreason 63e1fe3/b44df5d):
   EQ trims (+1.5 dB low @120, +0.4 dB mid @1800, −1.0 dB high @9000 — the
   high shelf DOWN tames harshness instead of polishing it later) and a
   brickwall limiter (thr −1.5 dB, 20:1, .003/.15) after the bus comp.
   The flat path (G29 neutral evidence) stays EXACTLY the pre-v0.27 graph. */
this.widthNet=null;this.widthOn=false;this.pp=null;this.ppOn=false;this._irKind='classic';if(this.masterFlat){this.master.connect(comp);comp.connect(an)}else{const lim=this.limiter=ctx.createDynamicsCompressor();lim.threshold.value=-1.5;lim.knee.value=0;lim.ratio.value=20;lim.attack.value=.003;lim.release.value=.15;comp.connect(lim);lim.connect(an)}an.connect(ctx.destination);this.noise=this.mkNoise();this.chains=[];this.scCache=[];this.duckState=[];this.duckEvents=0;this._plan={};for(let t=0;t<MAX_TRACKS;t++){const input=ctx.createGain();/* sidechain duck: ONE persistent GainNode per bus, created at init —
