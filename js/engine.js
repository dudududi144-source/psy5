...
import { ROM_TYPES, renderRomPcm } from '../foundation/dsp/perc-rom.mjs';
...
src.start(when);src.stop(when+endSec+.016);v.busyUntil=when+winSec;v.lastTrigger=++this._voiceSeq;this.romSpawns++;return true}
warmRom(types){let n=0;for(let i=0;i<types.length;i++){const t=types[i];if(!ROM_TYPES.has(t))continue;if(this.romCache.has(t+'@'+this.ctx.sampleRate))continue;if(this.romBuffer(t))n++}return n}
}
...
class RomVoice{constructor(ctx,eng){this.ctx=ctx;this.eng=eng;this.bus=null;this.g=ctx.createGain();this.g.gain.value=0;this.f=ctx.createBiquadFilter();this.f.type='highshelf';this.f.frequency.value=3200;this.f.gain.value=0;this.f.connect(this.g);this.busyUntil=0;this.lastTrigger=0}
