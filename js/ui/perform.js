...
function padHit(i){
  ...
  if(I.padMode==='DRUM'){
    ...
    if(m.mode==='chord'){/* v0.29.0 SPREAD VOICING (psyreason dc072ca): 5-voice stack — root+3+5+7 + octave-up 3rd — with ALTERNATING per-note stereo (odd indices left, even right; width from the pad preset's seeded `width` or .55 default) and rebalanced velocities (sides .55, sparkle .45 → summed level ≈ +8% of the 3-voice era, inside the glue comp's room). Non-root triggers carry lock.pan — the per-voice StereoPanner (engine.js SynthVoice) — the stereo width the pad grid never had. */const w=(tr.sound&&tr.sound.width!=null)?tr.sound.width:.55;m.notes.forEach((n,j)=>I.eng.trigger(tr,I.ctx.currentTime,{vel:j===0?vel:(j===4?vel*.45:vel*.55),note:n,lock:{pan:(j%2===0?1:-1)*w}},0));recHit(ti,m.notes[0],vel)}
...
function djFire(kind){const r=PERF.dj(kind);if(!r.ok){if(r.reason==='offline')toast('ENGINE OFFLINE — boot first');else toast((kind==='texture'?'SWELL':kind.toUpperCase())+' — no '+kind+' voice in this set. Assign one: Sound tab → drum type '+kind+' → ASSIGN')}return r.ok}

function wirePerform(){$('bFill').onclick=()=>{const t=PERF.fillCycle();$('bFill').textContent='⚡ FILL · '+FILL_NAMES[t];PERF.fill()};$('bVar').onclick=()=>PERF.variation();const dj=(id,kind)=>{const b=$(id);if(b)b.onclick=()=>djFire(kind)};dj('bRiser','riser');dj('bSwell','texture');dj('bImpact','impact');dj('bDown','downlifter');/* v0.21.0 THROW tools — arm/release is fully handled inside PERF.throwFx (honest toasts included) */const thr=(id,kind)=>{const b=$(id);if(b)b.onclick=()=>PERF.throwFx(kind)};thr('bThrowE','echo');thr('bThrowM','muffle');}
